...
internal fun MainActivity.setupBackupStatusStrip() {
    ...
    binding.topBar.post {
        val stripHeight = binding.topBar.height
                ...
                viewModel.backupStripState.collect { state ->
                    ...
                    when (state) {
                        ...
                        is BackupStripState.Running -> {
                            //Toast.makeText(myContext, "Strip: Running received, wasHidden=$wasHidden", Toast.LENGTH_SHORT).show()
                            ...
                            if (wasHidden || strip.root.visibility != View.VISIBLE) {
                                slideStripDown(strip.root, stripHeight)
                            }
