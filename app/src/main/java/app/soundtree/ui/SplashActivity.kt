...
import app.soundtree.util.OrphanRecordingScanner
 ...
 * [app.soundtree.ui.recovery.OrphanRecoveryDialogFragment] if the lists
 * are non-empty.
 */
class SplashActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val repo = (application as SoundTreeApp).repository

        lifecycleScope.launch(Dispatchers.IO) {
            val knownPaths = repo.getKnownFilePaths()
            val orphans      = OrphanRecordingScanner.scan(applicationContext, knownPaths)

            val lastOpenedAt = getSharedPreferences("soundtree_settings", Context.MODE_PRIVATE)
